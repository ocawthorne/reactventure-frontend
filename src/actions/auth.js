export const signup = (userData) => {
   return dispatch => {
      fetch(`http://http://reactventure.herokuapp.com/api/v1/users`, {
         method: 'POST',
         headers: {
...
export const login = (userData, history) => {
   return dispatch => {
      fetch("http://http://reactventure.herokuapp.com/api/v1/sessions", {
         method: "POST",
         headers: {
            "Content-Type": "application/json",
         },
...
export const checkLoggedIn = () => {
   return dispatch => {
      fetch(`http://http://reactventure.herokuapp.com/api/v1/logged_in`, {
         credentials: 'include'
      })
      .then(res => res.json())
      .then(data => dispatch({
...
export const logout = () => {
   return dispatch => {
      fetch(`http://http://reactventure.herokuapp.com/api/v1/logout`, {
         method: "DELETE",
         credentials: "include"
      })
      .then(res => res.json())
...
export const save = (user, hist=[], inventory=[], knownObjects=['crowbar','door','desk','drawer','paper','candle','chest'], brokenObjects=[], uniqueEvents={openedChest: false, meltedIce: false, completedGame: false}) => {
   return dispatch => {
      fetch(`http://http://reactventure.herokuapp.com/api/v1/users/${user.id}`, {
         method: 'POST',
         headers: {
            "Content-Type": "application/json",
         },
